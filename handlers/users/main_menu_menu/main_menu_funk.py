...
async def main_menu_start(call):
    await bot.send_message(chat_id=call.from_user.id, text=f"{await get_greetings_based_on_time()}, {call.from_user.full_name}!{greeting_text}", reply_markup=await create_main_menu_keyboard())
