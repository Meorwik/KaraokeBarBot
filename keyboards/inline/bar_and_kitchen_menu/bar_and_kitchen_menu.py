...
Bar_And_Kitchen_menu_button_callbacks = ["Bar_", "Kitchen_", "back_"]
OpenMenu_button_callbacks = ["back_"]

async def create_bar_button():
    bar_button = InlineKeyboardButton(text="Бар", callback_data=Bar_And_Kitchen_menu_button_callbacks[0])
    return bar_button
    
async def create_kitchen_button():
    kitchen_button = InlineKeyboardButton(text="Кухня", callback_data=Bar_And_Kitchen_menu_button_callbacks[1])
    return kitchen_button   

async def combine_bar_and_kitchen_menu_keyboard():
...
async def create_bar_and_kitchen_menu_keyboard():
    keyboard = await init_keyboard()
    list_with_buttons = await combine_bar_and_kitchen_menu_keyboard()
...
async def combine_open_menu_keyboard():
    OpenMenu_button = await create_OpenMenu_button()
    back_button = await create_back_button()
    return [OpenMenu_button, back_button]

async def create_open_menu_keyboard():
    keyboard = await init_keyboard()
    list_with_buttons = await combine_open_menu_keyboard()
    keyboard.add(*list_with_buttons)
    return keyboard
